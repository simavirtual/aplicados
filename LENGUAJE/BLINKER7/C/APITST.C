/*
    ...
    97.10.01    Blinker extender API demonstration program
...
void main (void)
{
   ...
   printf ("\nAPITST 4.2 Blinker extender API test program");
   printf ("\n--------------------------------------------\n\n");
   ...
   /* Preferred way to get a selector that corresponds to the screen buffer */
   ...
   /* Preferred way to get a selector for the BIOS Data area */
   ...
   /* Alternative way to get a protected mode pointer to the BIOS data area */

   if ((fp = DosRealToProt ((REALPTR) MK_FP (0x40,0))) == NULL)
      apitsterr ("Unable to map pointer to BIOS data area");

   printf ("Pointer to BIOS data area : %p\n\n",fp);
   if (DosGetSegDesc(SELECTOROF(fp),&desc_buf) == 0)
      {
      printf ("BIOS data area descriptor\n");
      printf ("-------------------------\n");
      printf ("Base address : %08lX\n",desc_buf.segbase);
   ...
   /* Create a real mode pointer to it from the protected mode pointer */

   if (rmptr = DosProtToReal (fp))
      printf ("Real mode pointer to BIOS data area : %p\n\n", rmptr);
