...
void main(void)
{
 ...
 char keystr [100];
   ...
   if (! SWPGETPID("swapdemo.c"))
      ...
      printf("Child process return string    : %Fs\n\n",SWPGETSTR());
      }
   else
      {
      printf("This is the second invocation of SWAPDEMO\n");
      printf("Please enter a message to return to the parent with code 255\n");
      gets (keystr);

      status = SWPSETSTR ("swapdemo.c","Value : ");
      status = SWPADDSTR ("swapdemo.c",keystr);
